...
function send(zipcode, area1, area2, area3, area4){
	...
	opener.document.updateForm.zipcode.value = zipcode; //opener는 updateForm
	var juso = area1 + " " + area2 + " " + area3 + " " + area4; //value값 선언할때 문자열더하기 안하는게 좋음.
	//opener.document.updateForm.address.value = "";
	opener.document.updateForm.address.value = juso; //opener는 updateForm
	window.close();
}
</script>
</head>
<body>
<form action="updatezipcheck.jsp" name="zipForm" method="post">
<table class="table">
	<tr><th>우편번호 찾기</th></tr>
	<tr>
		<td>
...
%>
				<a class="myButton" href="javascript:send('<%=zipcode %>', '<%=area1 %>','<%=area2 %>','<%=area3 %>','<%=area4 %>')"> <!-- 하이퍼텍스트처리로 감싸기 -->
