...
function makeCalendar(data) {
    ...
    for (i = 1; i <= lastDate.getDate(); i++) {
        cell = row.insertCell();
        if(arr.indexOf(i) != -1){
        	cell.innerHTML = i + `<br><img src="/resources/img/${data[arr.indexOf(i)].status}.png"/>` +'<br><small>'+ data[arr.indexOf(i)].maxTemp +'/'+ data[arr.indexOf(i)].minTemp +'</small><br>'+
        	 `<small style='color:blue'>${data[arr.indexOf(i)].sumRain == 0 ? '-' : `${data[arr.indexOf(i)].sumRain}`}<small>`;
        }else{
