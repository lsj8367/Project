...
public class DataController {
	...
	@Autowired
	private ObsWeatherService obsWeatherService;
	
	@Autowired
	private ShortForecastService shortForecastService;
	
	@Autowired
	private LongForecast3hService longForecast3hService;
	
	@Autowired
	private LongForecast6hService longForecast6hService;

	@Autowired
	private LongForecast12hService longForecast12hService;
	
	@Autowired
	private LongForecast24hService longForecast24hService;
	...
	public Map<String, Object> getPastWeather(@PathVariable("place") String place, @RequestBody Map<String, Object> shortInitDt){
		Map<String, Object> map = new HashMap<String, Object>();
		
		if(shortForecastService.getPastDayAccu(place, shortInitDt.get("shortInitDt").toString()) == null) {
			map.put("error", "error");
		}else {
			map.put("getPastDayAccu", shortForecastService.getPastDayAccu(place, shortInitDt.get("shortInitDt").toString()));			
		}
